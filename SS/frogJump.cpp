...
void solve(int sx, int sy, int dx, int dy, int ans){
   ...
   /*
    ...
    now in path 1 and path 2 b is present in both 
   ...
   */
    ...
    if(dp[sx][sy] > ans){
        ...
        for(int i = 0; i<4; i++){
            ...
            if(valid(x, y)){
            int temp;
            if(x == sx)temp = 0;//for horizontal jump x will remain same 
            if(y == sy)temp = 1;//for vertical jump y will remain same and we calculate for vertical jump
